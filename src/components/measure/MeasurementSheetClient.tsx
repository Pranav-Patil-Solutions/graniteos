...
import SiteMeasureSheet, { type Customer } from "@/components/measure/SiteMeasureSheet";
...
export default function MeasurementSheetClient({
  ...
  const [tab, setTab] = useState<"stock" | "site">("stock");
    ...
    <div className="px-4 pt-12 pb-8 max-w-4xl lg:max-w-6xl mx-auto">
      ...
      <div className="no-print mb-5">
        ...
        <p className="text-sm text-slate-400">
          {tab === "stock"
            ? "Every slab — size, area and value, from your stock."
            : "Type on-site measurements and print a PDF for a customer."}
        </p>
        <div className="mt-3 inline-flex rounded-xl border border-graphite-600 bg-white/[0.04] p-1">
          <button
            onClick={() => setTab("stock")}
            className={`px-3 py-1.5 text-sm font-semibold rounded-lg transition-colors ${
              tab === "stock" ? "bg-gold text-graphite-900" : "text-slate-300 hover:text-white"
            }`}
          >
            From stock
          </button>
          <button
            onClick={() => setTab("site")}
            className={`px-3 py-1.5 text-sm font-semibold rounded-lg transition-colors ${
              tab === "site" ? "bg-gold text-graphite-900" : "text-slate-300 hover:text-white"
            }`}
          >
            New measurement
          </button>
      ...
      {tab === "site" ? (
        <SiteMeasureSheet customers={customers} companyName={companyName} generatedAt={generatedAt} />
      ) : (
        <>
          {/* stock toolbar — screen only */}
          ...
          <div className="measure-print rounded-2xl border border-graphite-600 bg-white/[0.04] overflow-x-auto">
            ...
            {filtered.length === 0 ? (
              <p className="p-6 text-sm text-slate-400">
                {rows.length === 0 ? "No slabs in stock yet." : "No slabs match this filter."}
              </p>
            ) : (
