-- 0024: allow purchase_order / purchase_bill display numbers.
-- Extends the entity_type check on display_number_sequences and teaches
-- generate_display_number the PO / BILL prefixes. Body otherwise identical to 0022.

alter table public.display_number_sequences
  drop constraint if exists display_number_sequences_entity_type_check;
alter table public.display_number_sequences
  add constraint display_number_sequences_entity_type_check
  check (entity_type in ('quote','order','customer','inward','production','invoice','purchase_order','purchase_bill'));
...
begin
  ...
  v_prefix := case p_entity_type
    ...
    when 'purchase_order' then 'PO' when 'purchase_bill' then 'BILL'
    else 'DOC' end;
